...
if __name__ == '__main__':
    # 训练完成后，权重默认保存在 runs/train/exp/weights/best.pt
    model_path = os.path.join(ROOT, "runs", "train", "exp", "weights", "best.pt")
    model = YOLO(model_path) # 选择训练好的权重路径
    result = model.val(data=os.path.join(ROOT, "datasets", "data.yaml"),
